...
public class MoveMotherNatureTest {
    ...
    public void motherNatureShouldBeMoved() {
        ...
        PlayerBoard player = gameBoard.getTurnOrder().getCurrentPlayer();
        int randomMovement = new Random().nextInt(1, 7);
        IslandGroup initialPosition = gameBoard.getIslandField().getMotherNaturePosition();
        MoveMotherNature action = new MoveMotherNature(player.getId(), randomMovement);
        // act
        action.unsafeExecute(gameBoard);
        // assert
        assertEquals(Utils.modularSelection(initialPosition, gameBoard.getIslandField().getGroups(), randomMovement),
                gameBoard.getIslandField().getMotherNaturePosition());
    }
    ...
    public void playerCanMoveAccordingToChosenCard() {
        ...
        gameBoard.getTurnOrder().setSelectedCard(player, card, player.getAssistantCards());
    ...
    public void playerCantMoveMoreThanAllowed() {
        ...
        action.safeExecute(gh.getHistory(), gameBoard);
        // assert
        assertEquals(initialPosition, gameBoard.getIslandField().getMotherNaturePosition());
    }
}