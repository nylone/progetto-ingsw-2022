        ...
        switch (ctx.getClouds().size()) {
            case 2 -> { // two clouds will be print on top of each other
                ...
                for (Cloud c : ctx.getClouds()) {
                    ...
                    twoClouds.append("\tCloud ").append(c.getId()).append("\n").append(students);
                }
            ...
            case 3 -> { // three clouds will be print: the first pair next to each other, the third under the first
                ...
                threeClouds.append("Cloud ").append(ctx.getClouds().get(0).getId()).append("\t\t" // divider between clouds
                ).append("Cloud ").append(ctx.getClouds().get(1).getId()).append("\n"); // title of cloud 2
                ...
                threeClouds.append("Cloud ").append(ctx.getClouds().get(2).getId()).append("\n"); // title of cloud 3
            ...
            case 4 -> { // four clouds will be print as a grid
                ...
                fourClouds.append("Cloud ").append(ctx.getClouds().get(0).getId()).append("\t\t" // separation between clouds
                ).append("Cloud ").append(ctx.getClouds().get(1).getId()).append("\n"); // title of cloud 2
                for (int i = 0; i < 3; i++) { // prints the content of the I and II clouds next to each other if present
                ...
                fourClouds.append("Cloud ").append(ctx.getClouds().get(2).getId()).append("\t\t" // separation between clouds
                ).append("Cloud ").append(ctx.getClouds().get(3).getId()).append("\n"); // title of cloud 4
