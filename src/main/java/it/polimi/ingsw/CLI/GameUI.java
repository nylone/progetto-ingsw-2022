...
public class GameUI {
    ...
    public static String draw(GameBoard ctx) throws InvalidContainerIndexException, EmptyContainerException {
        ...
        String entrance = PlayerBoardUI.drawEntrance(ctx.getMutableTurnOrder().getMutableCurrentPlayer(), ctx);
        String towers = PlayerBoardUI.drawTowers(ctx.getMutableTurnOrder().getMutableCurrentPlayer(), ctx);
        for (PawnColour p : PawnColour.values()) {
            ...
            screen = screen + "\t\t\t" + PlayerBoardUI.drawDiningRoomRow(ctx.getMutableTurnOrder().getMutableCurrentPlayer(),
                    p);
            // This will print one teacher per row if present
            screen = screen + "\t" + PlayerBoardUI.drawTeacher(p, ctx.getMutableTurnOrder().getMutableCurrentPlayer(),
                    ctx);
